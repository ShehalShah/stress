...
class Stress:
    ...
    def run(self, input_video):
        vs = VideoStream(src=input_video).start()
        ...
        while True:
            ...
            rect = self.get_first_face(gray)

            if rect is not None:
                landmarks = self.get_landmarks(gray, rect)
                self.forehead, forehead_p1, forehead_p2 = self.get_forehead(frame, landmarks)

                if self.disp_forehead:
                    frame[forehead_p1[1]:forehead_p2[1], forehead_p1[0]:forehead_p2[0]] = self.forehead
                if self.disp_forehead_outline:
                    cv2.rectangle(frame, forehead_p1, forehead_p2, forehead_outline_color, forehead_outline_thik)
                ...
                if self.disp_rectangle:
                    self.draw_rectangle(frame, (x1, y1), (x2, y2), color=bbx_color, thikness=1, corners=60)
                ...
                if self.forehead is not None:
                    ...
                    frame = self.add_text_custom_font(frame, tmp_txt,
                                                       position=(text_left_margin, text_upper_margin), font=font,
                                                       color=font_color)
                    frame = self.add_text_custom_font(frame, "{:.2f}".format(self.hri),
                                                       position=(
                                                       text_left_margin + tmp_txt_size, text_upper_margin),
                    ...
                    tmp_txt_size += font.getsize(tmp_txt)[0]
                    frame = self.add_text_custom_font(frame, "{}".format(int(self.wait)),
                                                       position=(
                                                       text_left_margin + tmp_txt_size, text_upper_margin),
                                                       font=font_bold, color=font_color)
                    tmp_txt_size += font_bold.getsize("{}".format(int(self.wait)))[0]
                    frame = self.add_text_custom_font(frame, "s",
                                                       position=(
                                                       text_left_margin + tmp_txt_size, text_upper_margin),
                                                       font=font, color=font_color)
                    ...
                    frame = self.add_text_custom_font(frame, stress_txt,
                                                       position=(
                                                       text_left_margin,
                                                       text_upper_margin + space_text_line_upper + space_text_line_lower),
                                                       font=font_bold, color=font_color)

                    if self.disp_fps and self.fps is not None:
                        fps_txt = "{:.2f}fps".format(self.fps)
                        frame = self.add_text_custom_font(frame, fps_txt, position=(40, self.screen_height - 40),
                                                           font=font, color=font_color)
...
if __name__ == "__main__":
    stress = Stress()
    stress.run(input_video=0)  # Use 0 for webcam, or provide the path to a video file
